...
st.markdown(f"""<div style="font-size: 0.9em; margin-bottom: 20px;">누적: <span style="color: green;">익절 {sum(float(log.split(": ")[-1].replace(" USDT", "")) for log in st.session_state.logs if float(log.split(": ")[-1].replace(" USDT", "")) > 0):,.2f}</span> / <span style="color: red;">손절 {sum(float(log.split(": ")[-1].replace(" USDT", "")) for log in st.session_state.logs if float(log.split(": ")[-1].replace(" USDT", "")) <= 0):,.2f}</span> USDT</div>""", unsafe_allow_html=True)
...
st.markdown("""<div style="height: 50px;"></div>""", unsafe_allow_html=True) # 공간 고정
...
if st.session_state.auto_trading:
    ...
    if col_auto1.button("🟢 자동 매매 시작", use_container_width=True):
        ...
        st.session_state.msg = "🟢 자동 매매가 시작되었습니다."; st.session_state.msg_type = "success"
        st.rerun()
...
if b1.button("롱 진입", use_container_width=True):
    if amt <= st.session_state.balance:
        ...
        st.session_state.msg = "🟢 롱 포지션 진입 완료!"; st.session_state.msg_type = "success"
        st.rerun()
    else: st.session_state.msg = f"❌ 잔고 부족!"; st.session_state.msg_type = "error"; st.rerun()
...
if b2.button("숏 진입", use_container_width=True):
    if amt <= st.session_state.balance:
        ...
        st.session_state.balance -= amt
        st.session_state.msg = "🔴 숏 포지션 진입 완료!"; st.session_state.msg_type = "error"
        st.rerun()
    else: st.session_state.msg = f"❌ 잔고 부족!"; st.session_state.msg_type = "error"; st.rerun()
...
if b3.button("❌ 종료", use_container_width=True):
    for p in st.session_state.positions:
        ...
        st.session_state.balance += (p['margin'] + pnl)
    st.session_state.positions = []
    st.session_state.msg = "🔴 모든 포지션이 종료되었습니다."; st.session_state.msg_type = "error"
    st.rerun()
...
if not st.session_state.positions: st.write("보유 포지션 없음")
    ...
    for p in st.session_state.positions:
        st.write(f"{p['type']} | {p['entry']} | {p['lev']}x")
